...
Java8同样可以在类型声明前使用注解，这在Java8之前是不允许的，像：
```
@MyAnnotation String str = "YiFei"
```

可以为了各种目的来使用注解，最普遍的有以下几种：
...
一些的标准注解被称之为元注解：他们的目标是其他的注解并包含它们的信息。
  ...
  - `@Inherited`:默认情况下注解是不是会子类继承的。这个注解自动让继承了注解类的子类都可以继承。这个注解可以使用到类上。
  ...
  ```
  如果我们这样使用一个不能重复使用的注解，编译器将会抛个错误出来，像：`Duplicate annotation of no-repeatable type`
